...
def read_last_column_values(file_path):
    vals = []
    with open(file_path, "r", encoding="utf-8") as f:
        for line in f:
            s = line.strip()
            # Skip comment, separator, or empty lines
...
def average_rank(vals, target):
    ...
    if not idxs:
        return None
    return (idxs[0] + idxs[-1]) / 2 + 1  # average rank (1-based)
    ...
    with open(output_file, "w", encoding="utf-8") as out:
        if 0 in group_map:
            for i, fname in sorted(group_map[0], key=lambda t: (t[0] is None, t[0] if t[0] is not None else -1)):
                ...
                vals = read_last_column_values(path)
                if not vals:
                    continue
                target = vals[0]
