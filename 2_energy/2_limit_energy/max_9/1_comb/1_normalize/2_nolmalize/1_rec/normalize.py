...
def compute_second_last_replacement(target_norm, data, sign=1.0, tol=1e-12):
    sum_sq_except = np.sum(data**2) - data[-2]**2
    remaining = target_norm**2 - sum_sq_except
    if remaining < -tol:
        return None
    if remaining < 0:
        remaining = 0.0
    return np.sqrt(remaining) * sign
...
def main():
    ...
    for fname in sorted(os.listdir(args.indir)):
        ...
        data = data.copy()
        data[-3] = elev
        r = compute_second_last_replacement(T, data, sign=sign)
        if r is None:
            print(f"[ERROR] {fname}: no solution for second-to-last element", file=sys.stderr)
            sys.exit(1)
        data[-2] = r
        scaled = data / T
        ...
        final_norm = np.linalg.norm(scaled)
        print(f"[OK] {fname}: final_norm={final_norm:.6f}", file=sys.stderr)
