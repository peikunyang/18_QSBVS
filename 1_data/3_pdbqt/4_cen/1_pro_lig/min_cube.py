...
def main():
    ...
    out_dir = os.path.join(script_dir, "pdbqt")
    ...
    extra_t = np.array([-0.3, -0.3, -0.3], dtype=np.float64)
    lig_lines_1 = apply_transform(lig_lines_1, np.eye(3), extra_t)
    pro_lines_1 = apply_transform(pro_lines_1, np.eye(3), extra_t)

    out_lig = os.path.join(out_dir, lig_base)
    out_pro = os.path.join(out_dir, pro_base)
