...
def write_lines(p: str, lines: List[str]) -> None:
...
def replace_xyz_fixed(line: str, xyz: Tuple[float, float, float]) -> str:
    ...
    l[30:38] = list(f"{x:8.3f}"); l[38:46] = list(f"{y:8.3f}"); l[46:54] = list(f"{z:8.3f}")
    return "".join(l) + "\n"
...
def get_atom_indices_and_xyz(lines: List[str]) -> Tuple[List[int], np.ndarray]:
    ...
    if not pts: raise RuntimeError("No ATOM/HETATM parsed from file.")
    ...
    for line in lines:
        if line.startswith("ATOM") or line.startswith("HETATM"):
            try:
                ...
                v = (R @ v) + t
                out.append(replace_xyz_fixed(line, (v[0], v[1], v[2])))
...
def search_best_rotation(points: np.ndarray, levels=3, coarse_deg=15.0, fine_factor=3.0):
    ...
    for R0 in (np.eye(3), pca_basis(points).T):
        side, _ = cube_side_center(points @ R0.T)
        if side < best_side: best_side = side; best_angles = (0.0, 0.0, 0.0)
    step = math.radians(coarse_deg); center_angles = list(best_angles)
    for _ in range(levels):
        rx0, ry0, rz0 = center_angles
        cand = np.linspace(-step, step, 7)
        for drx in cand:
            for dry in cand:
                for drz in cand:
        ...
        center_angles = list(best_angles); step /= fine_factor
    return euler_xyz(*best_angles), best_side, best_angles

ap = argparse.ArgumentParser(description="Rotate/translate a ligand PDBQT to minimize its enclosing cube; output to script directory with same basename.")
ap.add_argument("--lig", required=True, help="Ligand PDBQT path")
...
R, _, ang = search_best_rotation(lig_pts, levels=args.levels, coarse_deg=args.coarse_step, fine_factor=args.fine_factor)
rot = lig_pts @ R.T
