...
final_offset = np.array([-0.3, -0.3, -0.3], dtype=np.float64)
lig_lines_1 = apply_transform(lig_lines_1, np.eye(3), final_offset)
_, lig_pts_after = get_atom_indices_and_xyz(lig_lines_1)

write_lines(out_lig, lig_lines_1)

mins = lig_pts_after.min(axis=0)
maxs = lig_pts_after.max(axis=0)
print("=== Summary ===")
